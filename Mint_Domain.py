...
from config import ZERO_ADDRESS, W_BERA, STG_USDC, BEX_W_BERA_STG_USDC_POOL, BEX_ADDRESS, BEX_ABI, HONEY_ADDRESS, HONEY_ABI, ERC20_ABI, BERADOMAIN_ADDRESS, BERADOMAIN_ABI
...
contract = w3.eth.contract(BERADOMAIN_ADDRESS, abi=BERADOMAIN_ABI)

def mint_domain(account):
    address = account.address
    nonce = w3.eth.get_transaction_count(address)
    domain = f'{Faker().word()}{random.randint(100, 999)}'
    transaction = contract.functions.mintNative([str(i) for i in domain], 1, address, 'https://beranames.com/api/metadata/69', address).build_transaction({
        'chainId': w3.eth.chain_id,
        'gas': 759346,
        'from': address,
        'nonce': nonce,
        'value': 360126764621146
    ...
    transaction_hash3 = w3.eth.send_raw_transaction(signed_swap_txn.rawTransaction)
    return transaction_hash3
