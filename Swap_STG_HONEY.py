...
contract = w3.eth.contract(HONEY_ADDRESS, abi=HONEY_ABI)

def swap2(account):
    address = account.address
    nonce = w3.eth.get_transaction_count(address)
    amount = round(random.uniform(0.01, 0.05), 4) #рандомное значение от 0.01, 0.05
    amount_out = int(amount * 10 ** 18) 

    transaction = contract.functions.mint(address, STG_USDC, amount_out).build_transaction({
    ...
    transaction_hash2 = w3.eth.send_raw_transaction(signed_swap_txn.rawTransaction)
    return transaction_hash2
