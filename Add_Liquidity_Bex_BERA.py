...
def add_liq(account):
    ...
    amount = random.randint(100000000000000, 1000000000000000) #рандомное значение от 0.0001 до 0.001
    amount_out = int(amount * 0.9)
    transaction = contract.functions.addLiquidity(BEX_W_BERA_STG_USDC_POOL, address, [ZERO_ADDRESS], [amount_out]).build_transaction({
...
with open(txt, 'r', encoding='utf-8') as keys_file:
    ...
    for account, proxy in zip(accounts, proxies):
        ...
        transaction_hash = add_liq(account)
        print(f'https://artio.beratrail.io//tx/{transaction_hash.hex()}')
