...
from config import ZERO_ADDRESS, W_BERA, STG_USDC, BEX_W_BERA_STG_USDC_POOL, BEX_ADDRESS, BEX_ABI, HONEY_ADDRESS, HONEY_ABI, ERC20_ABI, BERA_MIM_POOL, BERPS_ABI, BERPS_ADDRESS, BEND_ADDRESS, BEND_ABI
...
contract = w3.eth.contract(BERPS_ADDRESS, abi=BERPS_ABI)

def deposit_honey_berps(account):
    address = account.address
    nonce = w3.eth.get_transaction_count(address)
    amount = round(random.uniform(0.001, 0.003), 4) #рандомное значение от 0.0001 до 0.001
    amount_out = int(amount * 10 ** 18) 
    transaction = contract.functions.deposit(amount_out, address).build_transaction({
    ...
    transaction_hash6 = w3.eth.send_raw_transaction(signed_swap_txn.rawTransaction)
    return transaction_hash6
