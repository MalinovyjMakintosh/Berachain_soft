...
from config import ZERO_ADDRESS, W_BERA, STG_USDC, BEX_W_BERA_STG_USDC_POOL, BEX_ADDRESS, BEX_ABI, HONEY_ADDRESS, HONEY_ABI, ERC20_ABI, BEND_ADDRESS, BEND_ABI, AHONEY_ADDRESS, AHONEY2_ADDRESS, AHONEY2_ABI
...
contract = w3.eth.contract(AHONEY2_ADDRESS, abi=AHONEY2_ABI)

def withdraw2(account):
    address = account.address
    nonce = w3.eth.get_transaction_count(address)
    transaction = contract.functions.makeWithdrawRequest(2 ** 256 - 1).build_transaction({
    ...
    transaction_hash7 = w3.eth.send_raw_transaction(signed_swap_txn.rawTransaction)
    return transaction_hash7
